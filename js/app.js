...
(function() {
  ...
  function renderQuizView(quiz, isBonus) {
    ...
    const totalCount = (window.QuizEngine.quizzes && window.QuizEngine.quizzes.length) ? window.QuizEngine.quizzes.length.toLocaleString() : '7,250';
    if (isBonus) {
      quizIndexIndicator.textContent = `추천 상식 #${quiz.id} / ${totalCount}`;
    } else {
      quizIndexIndicator.textContent = `오늘의 상식 #${quiz.id} / ${totalCount}`;
    }
